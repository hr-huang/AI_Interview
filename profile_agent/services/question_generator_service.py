...
通用约束：
- 只问一个清晰的问题；
- 只保留一个主要回答目标，不要列出多个问题或问题清单，不要一次列出多个子问题，尤其不要堆叠彼此独立的子问题；
...
请让问题贴合 question_mode，围绕一个真实业务情景或一段真实经历，只保留一个主要问题，不要一次列出多个子问题（尤其不要堆叠彼此独立的子问题）。优先让候选人做一个关键判断，并用具体事实、取舍、失败边界或可量化验证来支撑回答。
