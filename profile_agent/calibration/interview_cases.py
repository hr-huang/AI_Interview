...
def _c01() -> InterviewCalibrationCase:
    return _case(
        ...
        [
            ...
            _rule(
                "C01_rag",
                ["RAG", "Context", "上下文", "检索", "记忆", "工具", "Tool"],
                "Context 保留当前目标、实时状态和最近两轮，较早对话压缩成带原始 turn_id 的结构化摘要；长期记忆按用户、时间戳和版本过滤后检索并重排，发生冲突时实时状态优先，过期记忆不进入上下文。RAG 结果必须带引用；工具调用前校验参数 Schema、用户权限和风险级别，调用后校验结果类型。我会评测召回率、引用正确率、知识过期和上下文污染。",
                max_uses=2,
            ),
            _rule(
                "C01_delivery",
            ...
            _rule(
                "C01_reliability",
                ["可靠性", "失败", "恢复", "重试", "安全", "评测", "人工", "风险", "监管", "合规", "容错", "降级", "幂等", "工具调用"],
                "我区分请求未送达、响应丢失、可重试和不可重试错误；每次调用先写入带幂等键、request_id 和版本号的调用意图，响应丢失时按同一幂等键查询结果而不是重复执行。不可重试错误走补偿或降级，高风险动作必须经过授权和人工确认；审计日志完整记录调用意图、审批人、参数摘要和结果，并用故障注入、重复写入率和人工接管成功率验收。",
                max_uses=10,
            ),
            _rule(
                "C01_evolution",
...
def _c04() -> InterviewCalibrationCase:
    return _case(
        ...
        [
            _rule(
                "C04_agent_boundary",
                ["状态", "节点", "汇合", "并行", "join", "Workflow", "编排", "动态路由"],
                "我比较过单 Agent、Workflow 和多 Agent：因为各分支并行但共享同一业务状态，所以选择 Workflow；只有职责和数据所有权真正独立时才拆成多 Agent。共享状态的 owner 放在编排器，节点只读写自己声明的字段；join 用 correlation id 聚合，超时、重复和部分失败都有明确状态。这样会增加字段映射，但能避免并发覆盖并支持重放。",
                max_uses=10,
            ),
            _rule(
                "C04_recovery_boundary",
                ["失败", "恢复", "重试", "幂等", "人工", "可靠性", "安全", "评测", "降级", "接管", "授权", "合规", "Checkpoint", "Tool Calling"],
                "失败按超时、可重试和不可重试分类，可重试错误指数退避两次并从 checkpoint 恢复，写入使用幂等键，不可重试错误执行补偿；工具输入输出通过 Schema、权限和风险级别校验，高风险动作进入人工确认。上线前用故障注入评测恢复成功率、重复写入率和人工接管延迟。只重试可恢复错误是为了避免放大业务错误，即使会增加延迟。",
                max_uses=10,
            ),
            _rule(
                "C04_unverified",
...
def _c05() -> InterviewCalibrationCase:
    return _case(
        ...
        [
            _rule(
                "C05_unsafe",
                ["高风险", "授权", "审批", "人工确认", "安全", "安全边界", "外部写入", "权限", "风险", "监管", "受监管", "合规", "确定性"],
