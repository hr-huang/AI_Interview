...
def _reason_view(
    ...
    *,
    ...
    enterprise_excerpts_by_evidence: Mapping[str, EvidenceExcerptView] | None = None,
) -> ReasonView:
    ...
    for evidence_id in reason.evidence_ids:
        ...
        source = (
            enterprise_excerpts_by_evidence.get(evidence_id)
            if enterprise_excerpts_by_evidence is not None
            else _source_for_evidence(
                evidence_id,
                evidences_by_id,
                turns_by_id,
                reason,
...
def _enterprise_evidence_excerpt_index(
    report: AssessmentReport,
    turns_by_id: Mapping[str, InterviewTurn],
) -> tuple[list[EvidenceExcerptView], dict[str, EvidenceExcerptView]]:
    """Project the enterprise excerpts once and retain their server-side join.

    Score reasons still carry raw evidence IDs on the server.  The browser
    receives only the safe excerpt fields, while this index lets every radar
    reason reuse the exact interpretation/limitation generated by the report
    assembler instead of rebuilding generic copy in the web layer.
    """

    views = _enterprise_evidence_excerpt_views(report, turns_by_id)
    by_evidence_id: dict[str, EvidenceExcerptView] = {}
    raw_ids_by_public_key: dict[tuple[str, str], list[str]] = {}
    for raw in report.enterprise_assessment.evidence_excerpts:
        raw_ids_by_public_key.setdefault((raw.turn_id, raw.quote), []).append(
            raw.evidence_id
        )
    for view in views:
        raw_ids = raw_ids_by_public_key.get((view.turn_id, view.quote), [])
        if raw_ids:
            by_evidence_id[raw_ids.pop(0)] = view
    return views, by_evidence_id
...
def _enterprise_assessment_view(
    ...
    *,
    dimension_labels: Mapping[str, str],
    evidence_excerpts: list[EvidenceExcerptView] | None = None,
    ...
    return EnterpriseAssessmentView(
        ...
        evidence_excerpts=(
            ...
            else _enterprise_evidence_excerpt_views(report, turns_by_id)
        ),
    )
    ...
    return JobMatchView(
        ...
        limiting_reasons=[
            _reason_view(
                ...
                enterprise_excerpts_by_evidence=enterprise_excerpts_by_evidence,
            )
...
def build_report_view(
    ...
    *,
    ...
    enterprise_evidence_views, enterprise_excerpts_by_evidence = _enterprise_evidence_excerpt_index(
        normalized_report,
        turns_by_id,
    )
    enterprise_assessment = _enterprise_assessment_view(
        ...
        evidence_excerpts=enterprise_evidence_views,
    )
