...
**Tech Stack:** Python 3.11+, Pydantic v2, pytest, 现有 qdrant-client、现有 SiliconFlow embedding client、fake embeddings、fake/local Qdrant、JSON/JSONL、PowerShell。
...
- [ ] 题目必须原创、情景化、可追问；review sidecar 记录人工审查者、日期、decision、六维/模式/证据检查和修改意见。expected_signals、critical_errors、follow_up_seeds 永不进入 embedding 或 candidate-safe prompt。
...
- profile_agent/services/qdrant_question_store.py
- profile_agent/scripts/run_question_bank.py
...
- [ ] **RED:** 在 tests/test_question_corpus_schema.py 为正文、QuestionBankManifest、QuestionSourceRegistry、review.json、dedupe.json、rights.json、locator.json 和 labeled intent 写最小合法 fixture；分别注入未知字段、重复主键、错误日期类型、空主键、非法 enum，运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_schema.py -q
  ~~~
...
- [ ] **RED:** 增加 policy/quotas 失败用例：第七模式、project 作为输出模式、重复 compatible、compatible 含 primary、dimension quota 不是 6/5/6/4/6/3、mode quota 不是 4/5/8/4/3/6、manifest count 不是 30。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_schema.py -k "policy or quota" -q
  ~~~
...
- [ ] **RED:** 回归 schema 与静态编译：
  ...
  ~~~powershell
  python -m pytest tests/test_question_rag_schema.py tests/test_question_corpus_schema.py -q
  python -m compileall profile_agent/schemas/question_rag_schema.py
  ~~~
...
- [ ] **RED:** 添加 v1 JSON、v2 JSON、旧 checkpoint 和 public projection fixtures，断言旧 question_mode 能读取，v1 缺省 compatible 为空，v2 能投影旧字段，未知 role/version 与非法 mode 被拒绝。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_bank_service.py tests/test_question_retrieval_service.py tests/test_question_rag_graph_integration.py -k "v1 or projection or mode" -q
  ~~~
...
- [ ] **RED:** 给 build_interview_graph 保持前五个位置参数、retrieval trace private field 和 public output 的兼容性写测试；运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_rag_graph_integration.py tests/test_rag_final_api_fix.py -q
  ~~~
...
- [ ] **RED:** 运行全兼容性回归：
  ...
  ~~~powershell
  python -m pytest tests/test_question_rag_schema.py tests/test_question_bank_service.py tests/test_question_retrieval_service.py tests/test_question_rag_graph_integration.py tests/test_rag_final_api_fix.py -q
  ~~~
...
- Modify profile_agent/services/question_bank_service.py
- Modify profile_agent/services/question_retrieval_service.py
- Modify profile_agent/services/qdrant_question_store.py
- Modify profile_agent/schemas/question_rag_schema.py
...
- [ ] **RED:** 为同一 record 的 JSON key 重排、skills/terms 重排、空白变化、非 embedding metadata 变化写 table-driven tests；断言六行键序严格为 question、business_constraint、skills、dimension_terms、primary_mode、compatible_modes，字段无换行；注入 difficulty、source、company、ID、score、JD/resume/answer/PII、expected signals 并断言不泄漏。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_bank_service.py tests/test_question_retrieval_service.py -k "embedding or text" -q
  ~~~
...
- [ ] **RED:** 为 hash 和 fingerprint 写测试：改变题干、business constraint、skill、dimension term、primary、compatible 会改变输入；改变非 embedding metadata 只改变 record content hash；改变 provider/model/vector dimension/text version/manifest hash/policy version 返回 index mismatch。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_bank_service.py tests/test_qdrant_question_store.py -k "hash or fingerprint" -q
  ~~~
...
- [ ] **RED:** 做 formatter、payload、fingerprint 的回归：
  ...
  ~~~powershell
  python -m pytest tests/test_question_bank_service.py tests/test_question_retrieval_service.py tests/test_qdrant_question_store.py -q
  git diff --check
  ~~~
...
- Modify profile_agent/services/question_bank_service.py
- Modify profile_agent/scripts/run_question_bank.py
...
- [ ] **RED:** 为 loader/validator 写最小合法 snapshot 和每类非法 snapshot：unknown field、duplicate PK、orphan question/source FK、wrong count/role/version、invalid mode/dimension、active low trust、bad hash/URL、source URL 超 3 题、missing review/rights/locator；运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_governance.py tests/test_run_question_bank.py -k "corpus or sidecar or audit" -q
  ~~~
...
- [ ] **RED:** 为证据时间窗写 boundary tests：每题 interview + independent official/JD，近180/近365计数，最多3 fallback、2025-01-01 下界、gap-only、例外理由、额外近180 official/JD、evergreen/current JD 的 verified date、至少12独立 URL及 URL association cap。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_governance.py -k "evidence or freshness or fallback" -q
  ~~~
...
- [ ] **RED:** 为 CLI 增加 read-only/dry-run tests，断言 audit-corpus、manifest 和 evaluate-local 不需要 embedding 环境变量、不触发 provider、不会调用 Qdrant 写操作；运行：
  ...
  ~~~powershell
  python -m pytest tests/test_run_question_bank.py -k "dry_run or audit or manifest" -q
  ~~~
...
- [ ] **GREEN:** 在 run_question_bank.py 增加 --corpus-dir 默认路径、audit-corpus、manifest、evaluate-local actions 与 --dry-run；输出 artifacts/question_corpus/validation_report.json 和 manifest_preview.json。保留既有 validate/audit/rebuild/sync 行为；--apply 仍是显式动作且本任务不执行。提交：
  ...
  ~~~powershell
  git add profile_agent/services/question_corpus_governance.py profile_agent/services/question_bank_service.py profile_agent/scripts/run_question_bank.py tests/test_question_corpus_governance.py tests/test_run_question_bank.py
...
- [ ] **RED:** 写四组 retrieval tests：primary 有候选时 compatible 不得参与；primary 为空但 compatible 有候选时 trace 标为 fallback；两者为空时 selected 为空且 status 为 no_match；跨 dimension/role、过期、retired、低 trust、excluded ID 的候选均被过滤。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_retrieval_service.py tests/test_qdrant_question_store.py -k "primary or compatible or no_match" -q
  ~~~
...
- [ ] **RED:** 写 generator prompt safety tests：candidate 只含 question、business_constraint、skills 和必要 dimension/mode context，不含 source URL/title/ID、score、rubric、company tags、JD/resume/answer/PII、expected signals、critical errors、follow-up seeds；运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_generator_service.py tests/test_question_rag_graph_integration.py -q
  ~~~
...
- [ ] **RED:** 做全 retrieval/graph 回归：
  ...
  ~~~powershell
  python -m pytest tests/test_question_retrieval_service.py tests/test_qdrant_question_store.py tests/test_question_generator_service.py tests/test_question_rag_graph_integration.py tests/test_rag_final_api_fix.py -q
  ~~~
...
- [ ] **GREEN:** 提交：
  ...
  ~~~powershell
  git add profile_agent/services/question_retrieval_service.py profile_agent/services/qdrant_question_store.py profile_agent/services/question_generator_service.py profile_agent/graphs/interview.py profile_agent/web/container.py tests/test_question_retrieval_service.py tests/test_qdrant_question_store.py tests/test_question_generator_service.py tests/test_question_rag_graph_integration.py
...
- [ ] **RED:** 先用真实联网浏览公开直接页面，建立 discovery ledger 的失败清单测试：搜索结果页、登录页、验证码、付费墙、不可访问转载、没有发布日期或无法人工定位的页面不得进入 registry。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_governance.py -k "source_registry" -q
  ~~~

  预期：registry 尚未存在，source acceptance tests 失败。

- [ ] **GREEN:** 实际验证并登记至少 12 个独立 canonical URL，来源只来自三类允许 source type；每条保存 source_id、canonical_url、title、source_type、published_at、verified_at、accessed_at、trust、lifecycle、question_ids、人工摘要和可复核 locator。每个 URL 关联不超过 3 题。对公开面试经历保存事实摘要而不是大段原文；官方文档/JD 保存与题目能力相关的人工摘要。绕过登录、验证码或付费墙的页面不得使用。
...
- [ ] **RED:** 用固定 fixture 测 registry canonicalization：去 tracking query、保留 scheme/host/path/必要 query；duplicate canonical URL、source type、日期、question_ids 超 cap、低 trust active、role/version 不一致必须失败。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_governance.py -k "canonical or registry" -q
  ~~~
...
- [ ] **RED:** 做 source freshness/count audit，预期若不足 18 题近180、27 题近365、12 URL 或 association cap 超限则返回确定 error codes：
  ...
  ~~~powershell
  python -m profile_agent.scripts.run_question_bank audit-corpus --corpus-dir profile_agent/knowledge/question_banks/ai_agent_engineer_2026_h2 --dry-run
  ~~~
...
- [ ] **RED:** 先添加完整配额矩阵测试；矩阵必须逐项包含以下 30 个 question_id、dimension_id、primary_mode，compatible_modes 必须由 Task 1 的 policy 校验生成：
  ...
  | d02 | q007 foundation; q008 scenario; q009 project_deep_dive; q010 coding; q011 follow_up |
  ...
  | d06 | q028 scenario; q029 system_design; q030 follow_up |

  运行：

  ~~~powershell
  python -m pytest tests/test_question_corpus_governance.py -k "matrix or content" -q
  ~~~
...
- [ ] **RED:** 为 30 题写 review/dedupe/rights/locator FK tests：每题一条 review，decision 必须 approved 或修订后 approved；每题 dedupe 记录必须记录 canonical text hash/near-duplicate decision；rights 必须记录原创声明、来源摘要边界和许可检查；locator 必须能指向 source_id 的页面 section/heading/date；所有 source_ids 必须在 registry。运行：

  ~~~powershell
  python -m pytest tests/test_question_corpus_governance.py -k "review or dedupe or rights or locator" -q
  ~~~

  预期：sidecar 字段缺失或关联未完成时失败。

- [ ] **GREEN:** 逐题完成人工审查和 sidecars；审查者核对六维、primary/compatible、原创性、证据独立性、情景约束、可追问性、candidate-safe 内容和 PII；source registry 更新 question_ids，确保至少 18/30 近180、27/30 近365，fallback 不超过3且每例写 exception_reason、gap-only、2025-01-01 后和额外近180 official/JD。用 loader 重算 content_hash 与 manifest hash，禁止手写伪 hash。
...
- [ ] **RED:** 做完整 corpus audit：
  ...
  预期：30 条、六维/模式配额、证据、URL cap、review/dedupe/rights/locator 全部未通过的项目逐条列出；通过前不生成索引。
...
- [ ] **RED:** 添加 labeled-intent schema/evaluation tests，要求 q001 到 q030 各有一个 intent_id；每行必须有 role/version、dimension、requested_mode、query_text、gold、acceptable、hard_negative、label_notes；gold 只能对应自身题目，acceptable 同维且 exact/compatible，hard-negative 要覆盖错误维度、错误模式、expired、retired、duplicate、wrong role、low trust。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_evaluation.py -q
  ~~~
...
- [ ] **RED:** 为 fake store 评测写测试：固定向量和候选顺序下计算 Recall@3、MRR@3、dimension Recall@3、hard-negative hits、invalid/duplicate count、trace coverage；缺 gold、duplicate、无 trace 或错误 tier 必须失败。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_evaluation.py -k "metrics or labels" -q
  ~~~
...
- [ ] **RED:** 运行 CLI label/evaluate-local tests，断言只读、不访问网络、不要求 provider env：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_evaluation.py tests/test_run_question_bank.py -k "evaluate or intent" -q
  ~~~
...
- [ ] **GREEN:** 将 evaluate-local 接入 run_question_bank.py，读取 corpus snapshot 与 JSONL，允许 --store fake 或 loopback local Qdrant test double，输出 artifacts/question_corpus/evaluation_local.json；提交：
  ...
  ~~~powershell
  git add profile_agent/knowledge/question_banks/ai_agent_engineer_2026_h2/retrieval_intents.jsonl profile_agent/services/question_corpus_evaluation.py tests/test_question_corpus_evaluation.py profile_agent/scripts/run_question_bank.py
...
- [ ] **RED:** 写 zero-cost tests，monkeypatch SiliconFlow/HTTP/provider 构造函数为 fail-fast；运行 fake store 30 queries，另在可用时连接 127.0.0.1 local Qdrant，不允许外部 host；断言任何网络/付费调用均被捕获。运行：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_zero_cost.py -q
  ~~~
...
- [ ] **RED:** 执行全部 zero-cost commands，预期在数据、schema、evidence、routing、metrics 任一不满足时退出非零：
  ...
  ~~~powershell
  python -m pytest tests/test_question_corpus_schema.py tests/test_question_corpus_governance.py tests/test_question_corpus_evaluation.py tests/test_question_corpus_zero_cost.py -q
  python -m profile_agent.scripts.run_question_bank audit-corpus --corpus-dir profile_agent/knowledge/question_banks/ai_agent_engineer_2026_h2 --dry-run
  python -m profile_agent.scripts.run_question_bank evaluate-local --corpus-dir profile_agent/knowledge/question_banks/ai_agent_engineer_2026_h2 --store fake --dry-run
  ~~~
...
- [ ] **RED:** 在执行任何真实 embedding 前运行全仓相关验证：
  ...
  ~~~powershell
  python -m pytest tests/test_question_rag_schema.py tests/test_question_bank_service.py tests/test_question_retrieval_service.py tests/test_qdrant_question_store.py tests/test_run_question_bank.py tests/test_question_generator_service.py tests/test_question_rag_graph_integration.py tests/test_question_corpus_schema.py tests/test_question_corpus_governance.py tests/test_question_corpus_evaluation.py tests/test_question_corpus_zero_cost.py -q
  python -m compileall profile_agent
  git diff --check
  ~~~
...
- [ ] **GREEN:** 在 STOP 后立即结束本计划的自动执行；不要预先执行 --apply、联网付费 embedding、重试/调参或生产 Qdrant 写入。只有用户在看到 STOP 后再次明确批准，才另开受授权任务，先复述一次约 60 向量 BGE-M3 单批范围和 provider/model/dimension/fingerprint，再由用户选择是否运行。提交最终 verification artifact：
